...
class Withdrawals {
    ...
    setWithdrawCallback(withdraw, callback, cb) {
        if (withdraw.status != callback.status) {
            ...
            new WithdrawData().update(withdraw.id, withdraw.externalId, withdraw.status, withdraw.invoice, withdraw.amount, withdraw.fee, withdraw.processedDate, null).then(() => cb(null, withdraw)).catch(e => cb(e));
        } else {
            cb(null, false);
        }
    }
    ...
    getCreateResult(invoice, amount, username, cb) {
        new Payments().listImagePayments(username, null, (e, p) => {
            if (e) {
                cb(e);
            } else {
                if (!p || p.length == 0) {
                    ...
                    lock.acquire(username).then(() => {
                        new WithdrawData().list(username).then(w => {
                            ...
                            if (sameInvoice) {
                                ...
                                new WithdrawData().insert(withdraw.username, withdraw.status, withdraw.amount, withdraw.invoice).then((result) => {
                                    ...
                                    try {
                                        ...
                                        axios.post(url, body, httpConfig).then(response => {
                                            ...
                                            new WithdrawData().update(withdraw.id, withdraw.externalId, withdraw.status, withdraw.invoice, withdraw.amount, withdraw.fee, withdraw.processedDate, "starting").then(() => {
                                                lock.release(username);
                                                cb(null, {type: 'Withdraw', referenceDate: new Date(), imageId: null, status: withdraw.status, amount: withdraw.amount});
                                            }).catch(error => {
                                                lock.release(username);
                                                cb(error);
                                            });
                ...
                if (!p || p.length == 0) {
                    ...
                    new WithdrawData().list(username).then(w => {
                        ...
                        p.forEach(c => {
                            ...
                            balance.push({type: 'Payment', referenceDate: c.updatedAt, imageId: c.imageId, status: 'paid', amount: c.amount});
                        });
                        w.forEach(c => {
                            withdrawalAmount += c.amount;
                            balance.push({type: 'Withdraw', referenceDate: c.updatedAt, imageId: null, status: c.status, amount: c.amount});
                        });
